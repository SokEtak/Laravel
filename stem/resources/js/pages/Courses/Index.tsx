...
const getColumns = (
    ...
    const columns: ColumnDef<Course>[] = [
        ...
        },
        {
            ...
            header: "Description",
            cell: ({ row }) => <div className="px-3">{row.getValue("description") || 'N/A'}</div>,
        },
        {
            accessorKey: "level",
            header: ({ column }) => {
            ...
            cell: ({ row }) => <div className="px-3 capitalize">{row.getValue("level")}</div>,
            filterFn: "equals",
            enableSorting: false,
        },
        {
            ...
            header: ({ column }) => {
                ...
                return (
                    <div className="flex items-center space-x-2">
                        <span>Teacher</span>
                        <DropdownMenu open={isTeacherDropdownOpen} onOpenChange={setIsTeacherDropdownOpen}>
                            <TooltipProvider>
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                        <DropdownMenuTrigger asChild>
            ...
            cell: ({ row }) => <div className="px-3">{row.original.teacher ? `${row.original.teacher.first_name} ${row.original.teacher.last_name}` : 'N/A'}</div>,
            filterFn: (row, columnId, filterValue) => {
                const teacherName = row.original.teacher ? `${row.original.teacher.first_name} ${row.original.teacher.last_name}`.toLowerCase() : '';
                return teacherName.includes(String(filterValue).toLowerCase());
            },
            enableSorting: false,
        },
        {
            ...
            cell: ({ row }) => <div className="px-3">{row.original.subject?.name || 'N/A'}</div>,
            filterFn: (row, columnId, filterValue) => {
...
function Index() {
    const { flash, courses, isSuperAdmin, availableTeachers, availableSubjects, availableTenants } = usePage<PageProps>().props;
    ...
    const columns = useMemo(() => getColumns(
        ...
        availableLevels,
        availableTenants
    ), [setCourseToDelete, processing, isSuperAdmin, availableTeachers, availableSubjects, availableLevels, availableTenants]);
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="p-4 sm:p-6 lg:p-5 xl:p-2">
                        ...
                        <div className="flex items-center gap-2">
                            {isTableLoading ? (
                                ...
                                <span className="text-sm font-medium">
                                    {`${table.getFilteredRowModel().rows.length} filtered out of ${courses.length} courses`}
                                </span>
                            )}
                        </div>
                ...
                <AlertDialog open={!!courseToDelete} onOpenChange={(openState) => !openState && setCourseToDelete(null)}>
                    <AlertDialogContent>
                        <AlertDialogHeader>
