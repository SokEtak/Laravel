...
const getColumns = (
    setScheduleToDelete: React.Dispatch<React.SetStateAction<Schedule | null>>,
    processing: boolean,
    isSuperAdmin: boolean,
    availableTenants: TenantOption[],
    availableDays: string[],
    isDayDropdownOpen: boolean,
    setIsDayDropdownOpen: React.Dispatch<React.SetStateAction<boolean>>,
    ...
    const columns: ColumnDef<Schedule>[] = [
        ...
        },
        {
            ...
            cell: ({ row }) => <div className="px-3">{row.getValue("start_time")}</div>,
        },
        {
            ...
            cell: ({ row }) => <div className="px-3">{row.getValue("end_time")}</div>,
        },
        {
            ...
            cell: ({ row }) => <div className="px-3 capitalize">{row.getValue("day_of_week")}</div>,
            filterFn: "equals",
            enableSorting: false,
        },
    ];
...
function Index() {
    const { flash, schedules, isSuperAdmin, availableTenants } = usePage<PageProps>().props;
    ...
    const table = useReactTable({
        ...
        globalFilterFn: (row, columnId, filterValue) => {
            ...
            const dayOfWeek = String(row.original.day_of_week).toLowerCase();
            return dayOfWeek.includes(search);
        },
        state: {
            sorting,
            columnFilters,
    ...
    }, [flash.message, schedules]);

    const handleCloseAlert = () => setShowAlert(false);

    const confirmDelete = () => {
        if (scheduleToDelete) {
            destroy(route("schedules.destroy", scheduleToDelete.id), {
                onSuccess: () => {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="p-4 sm:p-6 lg:p-5 xl:p-2">
                        ...
                        <div className="flex items-center gap-2">
                            {isTableLoading ? (
                                ...
                                <span className="text-sm font-medium">
                                    {`${table.getFilteredRowModel().rows.length} filtered out of ${schedules.length} schedules`}
                                </span>
                            )}
                        </div>
                ...
                <AlertDialog open={!!scheduleToDelete} onOpenChange={(openState) => !openState && setScheduleToDelete(null)}>
                    <AlertDialogContent>
                        <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                                ...
                                <strong>
                                    {scheduleToDelete?.day_of_week} from {scheduleToDelete?.start_time} to {scheduleToDelete?.end_time}
                                </strong>.
                            </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
